...
if ( $COLD_START_INITIAL_CONDITIONS_MODEL == GFS || $COLD_START_INITIAL_CONDITIONS_MODEL == GFS_new ) then # Change if GFS after May 2016 to 32 levels
   setenv num_ungrib_vertical_levels  51 #27
else if ( $COLD_START_INITIAL_CONDITIONS_MODEL == GFS_FV3 ) then # GFS FV3
   setenv num_ungrib_vertical_levels  50 # Could be as many as 50
...
setenv num_ungrib_soil_levels  9 #atj: changed to 6 and used 4 for LBC data
...
ln -sf ${MPAS_GRID_INFO_DIR}/${graph_info_prefx}* .  # link graph files
...
foreach iter ( 1 2 3 4 ) 
   ...
   if ( $iter == 1 ) then
      ...
      setenv config_input_name    $grid_file_netcdf       # atj: needs to be lower case
	 ...
	 setenv config_vertical_grid   .true. #atj: set to true: also not sure it matters, but tutorial leaves them set when doing lbcs
	 setenv config_met_interp      .true.
	 setenv config_input_sst       .false.
	 setenv config_frac_seaice     .true.
	 ...
	 set suffix = "met_data_lbcs"
      ...
      echo "Iter = $iter is wrong.  Should be <= 4.  Big bug somehwere.  Exit"
      exit 0
   endif
   ...
   #$run_cmd ./init_atmosphere_model  #atj: not running with MPI yet
